...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------------
wells<- data.table::fread("Prepared_Surveys/master_survey_update2.csv")
...
op_alias<-fread("enverus_prism_well_header_alias/env_csv-Wells-2396d_2024-08-05.csv")
op_alias<-op_alias%>%select(Unformatted_API_UWI,ENVOperator)%>%distinct(Unformatted_API_UWI,.keep_all = T)%>%
  rename(ENV_Operator=ENVOperator,UWI_10=Unformatted_API_UWI)


wells<-wells%>%select(-ENV_Operator)%>%left_join(op_alias)

wells<-wells%>%mutate(LandingZoneBulk=ifelse(LandingZoneBulk=='WFMP SD','WFMP A',
                                             ifelse(LandingZoneBulk=='BS1SH','BS1S',LandingZoneBulk)))


wells<-wells%>%rename(WELLNAMENUMBER=Name)
wells<-wells%>%rename(COMPLETION_DATE=ComplDate,SPUD_DATE=SpudDate)
...
###Test Filter----
#wells<- wells%>%filter(grepl("DOMINATOR",WELLNAMENUMBER))
 

wells_survey_header_keep<-wells%>%select(UWI:County,COMPLETION_DATE:SPUD_DATE,meanTVD:STATION_LONG,X,Y,LandingZoneBulk,LandingZone,ENV_Operator,SrvyName)%>%
  relocate(ENV_Operator,.before=Operator)
  

# ###snowflake add enverus current operator
# myconn_out <- dbConnect(odbc::odbc(), "DRM_snowlflake")
# 
# tables <- dbGetQuery(con, "SELECT TABLE_NAME 
#                            FROM L48_PRD 
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------------
wells <- wells %>% rename(longitude = X, latitude = Y)
...
hz_buff <-st_as_sf(lines.shp)%>%st_buffer(dist=unique(lines$SpacingGeo),endCapStyle = 'FLAT')
...
midpoint<-st_line_sample(st_as_sf(lines.shp), sample=0.5)
midpoint<-st_as_sf(midpoint)
midpoint<-cbind(st_drop_geometry(st_as_sf(lines.shp)), midpoint)
midpoint<- st_as_sf(midpoint)
...
dist_outputfinal <-
  dist_outputfinal %>% filter(!(
    paste(WELLNAMENUMBER) == paste(WELLNAMENUMBER.1)
  ))
...
tstt<-tst%>%select(UWI, WellborePosition.1,length_est)%>%distinct(UWI,.keep_all = T)

bearing_df <- bearing_df %>% left_join(tstt) %>% distinct()
...
tst<- tst%>%filter(PercOverlap>.15, PercIntersect>.05)
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------------
# zone<- fread("zone.csv")
# zone<- zone%>%mutate(Reservoir=LANDING_ZONE)
# 
...
###New landing zone link

zone=wells_survey_header_keep%>%select(UWI,LandingZone,LandingZoneBulk,SrvyName)%>%distinct(UWI,.keep_all=T)%>%mutate(UWI=as.character(UWI))%>%rename(Reservoir=LandingZone)

left = function (string,char){ substr(string,1,char) }

gbd<-left_join(gbd %>%mutate(UWI=as.character(UWI)),zone%>%mutate(UWI=as.character(UWI))%>%rename(LandingZone=Reservoir))
...
tst<- tst%>%rename(Zone=Reservoir.x,Zone.1=Reservoir.y,ZoneBulk=LandingZoneBulk.x,ZoneBulk.1=LandingZoneBulk.y,
                   SurveyOrigin=SrvyName.x,Offset_SurveyOrigin=SrvyName.y)

tst<- rename(tst,c.Hypotenuse=c.hypotenuse)

tst$WELLNAMENUMBER<- as.character(tst$WELLNAMENUMBER)
tst$WELLNAMENUMBER.1<- as.character(tst$WELLNAMENUMBER.1)
...
tst <- tst %>% mutate(ZoneAlias = paste(basin, ZoneBulk, sep = ""))
tst <- tst %>% mutate(ZoneAlias.1 = paste(basin, ZoneBulk.1, sep = ""))

tst <-
  tst %>% group_by(UWI, PointInPolyUWI) %>% distinct(UWI, PointInPolyUWI, .keep_all =
                                                  T)
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------------
well_header<-well_header%>%filter(!is.na(X)|!is.na(Y))

well_header_sf<- st_as_sf(well_header,coords=c("X","Y"))
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------------
cluster_df_int <-
  cluster_df_int %>% mutate(Clust_ID = paste(Operator, "_", OBJECTID_1, sep ="")) %>%
  group_by(Clust_ID) %>%
...
spacing <-
  spacing %>% mutate(Parent_Child = ifelse(
    COMPLETION_DATE.x - COMPLETION_DATE.y > 120 & Hypotenuse <2500,
    "Child",
    ifelse(
      COMPLETION_DATE.x - COMPLETION_DATE.y < 121 &
        COMPLETION_DATE.x -
        COMPLETION_DATE.y > -121  & Hypotenuse <2500,
      "Development",
      ifelse(COMPLETION_DATE.x -
               COMPLETION_DATE.y < -120, "Parent", NA)
    )
  ))
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------------
spacing<- spacing%>%filter(PercOverlap>.15,Horizontal_Dist<2001)

spacing<- spacing%>%filter((DeltOrder<2|is.na(DeltOrder)))
spacing<- spacing%>%filter(!(is.na(DeltOrder) & TVDDiff>500))
...
nearest_well_L150tvd_2<-nearest_well_L150tvd%>%filter(TVDDiff<150)%>%
...
nearest_well_L150tvd_2Max90Day<-nearest_well_L150tvd%>%filter(TVDDiff<150 & Date_Diff > -91)%>%
	group_by(UWI,Offset_Direction_Wellbore)%>% 
	filter(Hypotenuse==min(Hypotenuse))%>%
	distinct(Offset_Direction_Wellbore,.keep_all=TRUE)%>%
	filter(Hypotenuse>100)%>%ungroup()%>%
...
nearest_well_L150tvd_2Max180Day<-nearest_well_L150tvd%>%filter(TVDDiff<150 & Date_Diff > -181)%>%
	group_by(UWI,Offset_Direction_Wellbore)%>% 
	filter(Hypotenuse==min(Hypotenuse))%>%
	distinct(Offset_Direction_Wellbore,.keep_all=TRUE)%>%
	filter(Hypotenuse>100)%>%ungroup()%>%
...
nearest_well_L150tvd_2Max270Day<-nearest_well_L150tvd%>%filter(TVDDiff<150 & Date_Diff > -271)%>%
	group_by(UWI,Offset_Direction_Wellbore)%>% 
	filter(Hypotenuse==min(Hypotenuse))%>%
	distinct(Offset_Direction_Wellbore,.keep_all=TRUE)%>%
	filter(Hypotenuse>100)%>%ungroup()%>%
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------------
nearest_well<- spacing%>%filter(PercOverlap>.15,Horizontal_Dist<2001)
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------------
same_zone<- spacing%>%filter(PercOverlap>.15, Zone==Zone.1|TVDDiff<150, Hypotenuse>300,Horizontal_Dist<2001)
...
same_zone_L500tvd<-same_zone%>%filter(TVDDiff<500)%>%
	group_by(UWI,Offset_Direction_Wellbore)%>% 
	filter(Hypotenuse==min(Hypotenuse))%>%
	distinct(Offset_Direction_Wellbore,.keep_all=TRUE)%>%
	filter(Hypotenuse>100)%>%ungroup()%>%
...
same_zone_L500tvd_90Day<-same_zone%>%filter(TVDDiff<500 & Date_Diff > -91)%>%
	group_by(UWI,Offset_Direction_Wellbore)%>% 
	filter(Hypotenuse==min(Hypotenuse))%>%
	distinct(Offset_Direction_Wellbore,.keep_all=TRUE)%>%
	filter(Hypotenuse>100)%>%ungroup()%>%
...
same_zone_L500tvd_180Day<-same_zone%>%filter(TVDDiff<500 & Date_Diff > -181)%>%
	group_by(UWI,Offset_Direction_Wellbore)%>% 
	filter(Hypotenuse==min(Hypotenuse))%>%
	distinct(Offset_Direction_Wellbore,.keep_all=TRUE)%>%
	filter(Hypotenuse>100)%>%ungroup()%>%
...
same_zone_L500tvd_270Day<-same_zone%>%filter(TVDDiff<500 & Date_Diff > -271)%>%
	group_by(UWI,Offset_Direction_Wellbore)%>% 
	filter(Hypotenuse==min(Hypotenuse))%>%
	distinct(Offset_Direction_Wellbore,.keep_all=TRUE)%>%
	filter(Hypotenuse>100)%>%ungroup()%>%
...
prep_final_tbl<- prep_final_tbl%>%group_by(UWI,Offset_Direction_Wellbore)%>% 
	filter(Hypotenuse==min(Hypotenuse))%>%ungroup()%>%arrange(Hypotenuse)
prep_final_tbl<- prep_final_tbl%>%ungroup()%>%group_by(UWI,Offset_Direction_Wellbore)%>%
distinct(PointInPolyUWI,.keep_all=TRUE)
...
prep_final_tbl <- prep_final_tbl %>%
  left_join(
    combined_90day %>%
      ...
      rename(
        ENV_Operator90Day=ENV_Operator,
        OPERATOR_SHORT90Day=Operator,
        WELLNAMENUMBER90Day=WELLNAMENUMBER,
        ENV_Operator.190Day=ENV_Operator.1,
        OPERATOR_SHORT.190Day=Operator.1,
        ...
        LandingZone=Zone,
        LandingZoneBulk=ZoneBulk,
        Offset_LandingZone=Zone.1,
        Offset_LandingZoneBulk=ZoneBulk.1,
      ),
    by = c(
      "UWI" = "UWI",
...
prep_final_tbl <- prep_final_tbl %>%
  left_join(
    combined_180day %>%
      ...
      rename(
        ENV_Operator180Day=ENV_Operator,
        OPERATOR_SHORT180Day=Operator,
        WELLNAMENUMBER180Day=WELLNAMENUMBER,
        ENV_Operator.1180Day=ENV_Operator.1,
        OPERATOR_SHORT.1180Day=Operator.1,
...
prep_final_tbl <- prep_final_tbl %>%
  left_join(
    combined_270day %>%
      ...
      rename(
        ENV_Operator270Day=ENV_Operator,
        OPERATOR_SHORT270Day=Operator,
        WELLNAMENUMBER270Day=WELLNAMENUMBER,
        ENV_Operator.1270Day=ENV_Operator.1,
        OPERATOR_SHORT.1270Day=Operator.1,
...
final_table <- final_table %>% rename(
  ...
  SurveyOrigin=SurveyOrigin.x,
  Offset_SurveyOrigin=SurveyOrigin.y
)
...
unfiltered_final_table <-
  tst_final %>% mutate(Parent_Child = ifelse(
    COMPLETION_DATE.x - COMPLETION_DATE.y > 120,
    "Child",
    ifelse(
      COMPLETION_DATE.x - COMPLETION_DATE.y < 121 &
        COMPLETION_DATE.x -
        COMPLETION_DATE.y > -121,
      "Development",
      ifelse(COMPLETION_DATE.x -
               COMPLETION_DATE.y < -120, "Parent", NA)
    )
...
unfiltered_final_table <- unfiltered_final_table %>% select(-PercIntersect) %>%
  rename(
    API12 = UWI,
    Operator_Short = Operator,
    WELLNAMENUMBER = WELLNAMENUMBER,
    ...
    Area = Area,
    Offset_Well_ID = PointInPolyUWI,
    Offset_ENV_Operator=ENV_Operator.1,
    Offset_Operator_Short = Operator.1,
