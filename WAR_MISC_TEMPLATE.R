...
your_shapefile_if_needed<-read_sf("path_to_file")


potash<-read_sf("your_potash_outline")
...
ENV_RIGS<- "your_rigs_sheet"

ENV_PERMITS <- "your_permits_sheet"


#original keep in case enverus add submit date
ENV_COMPLETIONS <- "your_completions_sheet"



ENV_DUCS <- "your_ducs_sheet"

ENV_ACTIVITY_FRAC_CREW<- "your_frac_crews_sheet"
...
basin_lines<-"your_basin_outline"
...
lapply("YOUR_AREAS",completions_map_fctn)
...
ggsave(plot=last_plot(),'your_path_.png',
...
ggsave(plot=last_plot(),'your_path_rig.png',
...
ppt<- read_pptx("your_ppt_template")
